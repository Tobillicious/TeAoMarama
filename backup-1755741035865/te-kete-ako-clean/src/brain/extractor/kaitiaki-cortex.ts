 ...
 *;
 * Like the prefrontal cortex in a human brain, this system: '* - Makes decisions about content structure;
...
const config = {server: '{;
  ...
  ai: {provider: (process.env.LLM_PROVIDER || deepseek").toLowerCase(),;
...
// Validation;
if (!config.ai.deepseekKey) {console.error(❌ Missing DEEPSEEK_API_KEY - this is our primary thinking engine!");
  process.exit(1)};
const app = express();
...
const _limiter = rateLimit({windowMs: 15 * 60 * 1000, // 15 minutes;
...
// Request logging;
app.use((req: Request, res: Response, next: 'NextFunction) => {const _start = Date.now();
  res.on("finish, () => {;
 ...
 */;
function buildExtractionPrompt(): unknown {chunkText: string, sourceUrl?: string, sourceTitle?: string, context?: unknown): string {return `You are Kaitiaki Aronui, the guardian of educational knowledge for Te Kete Ako, a platform serving Māori and Pacific students in New Zealand.;
...
EXTRACT from this content chunk: ""${chunkText.replace(/\\\"/g, "\\"")}";
SOURCE CONTEXT: - Title: ${sourceTitle || "Unknown};
- URL: ${sourceUrl || Not provided"};
- Context: ${JSON.stringify(context || {})};
EXTRACTION REQUIREMENTS: 1. KNOWLEDGE NODES - Extract discrete educational concepts:;
   Types: "nzc_outcome, unit", "lesson, activity", "resource, assessment", "cultural_protocol;
   For each node return: {id_hint": "unique-identifier,;
...
2. KNOWLEDGE EDGES - Relationships between concepts: Types: "prerequisite, supports", "extends, contradicts", "cultural_parallel, assessment_of";
   For each edge return: {"source_hint: source-node-id",;
     ...
     confidence": 0.9};
3. CULTURAL CONSIDERATIONS: - Flag content that mentions Māori culture, concepts, or language for cultural review;
...
4. QUALITY INDICATORS: - Mark content requiring teacher review;
 ...
 */;
async function callThinkingEngine(): 'unknown {prompt: string): Promise<string> {try {;
    ...
    const _content = response.data.choices?.[0]?.message?.content;
    if (!content) {throw new Error("No content in AI response)};
    return content;
  } catch(error: unknown) {console.error(🧠 Thinking engine error:", error.response?.data || error.message);
 ...
 */;
function parseAIResponse(): unknown {rawResponse: string): unknown {try {;
    // First, try direct parsing;
    const parsed = JSON.parse(rawResponse);
    // Validate required structure;
    if (!parsed.nodes || !Array.isArray(parsed.nodes)) {;
      throw new Error(Missing or invalid nodes array)};
    if (!parsed.edges || !Array.isArray(parsed.edges)) {throw new Error("Missing or invalid edges array")};
    // Validate node structure;
    for (const node of parsed.nodes) {if (!node.id_hint || !node.type || !node.title) {;
        throw new Error(`Invalid node structure: ${JSON.stringify(node)}`);
      };
    };
    return parsed;
  } catch(parseError) {console.warn(Direct JSON parse failed, attempting extraction..);
    // Try to find JSON within the response;
    const _jsonMatch = rawResponse.match(/\{[\s\S]*\}/);
    if (jsonMatch) {try {return JSON.parse(jsonMatch[0])} catch(extractError) {throw new Error(`JSON extraction failed: ${extractError}`);
      };
    };
 ...
 */;
async function logExtractionJob(): unknown {input: unknown, output: unknown, status: string, error?: string): Promise<void> {if (!supabase) return;
  try {;
  ...
  } catch(dbError) {console.warn(Failed to log extraction job: , dbError)};
};
// ========================================;
 ...
 */;
app.post("/extract", async (req: Request, res: Response) => {const _startTime = Date.now();
  try {;
    const { chunkText, sourceUrl, sourceTitle, context} = req.body;
    // Validation;
    if (!chunkText || typeof chunkText !== string) {return res.status(400).json({;
    ...
    if (chunkText.length < 50) {return res.status(400).json({;
        error: "chunkText too short - need at least 50 characters for meaningful extraction"});
    };
    if (chunkText.length > 50000) {return res.status(400).json({;
    ...
    const _prompt = buildExtractionPrompt(chunkText, sourceUrl, sourceTitle, context);
    // Think!;
    const _aiResponse = await callThinkingEngine('prompt);
    // Parse and validate;
    const parsed = parseAIResponse(aiResponse);
    // Log successful extraction;
    const _processingTime = Date.now() - startTime;
    await logExtractionJob({sourceTitle, sourceUrl, contentLength: chunkText.length},;
  ...
  } catch(error: 'unknown) {const _processingTime = Date.now() - startTime;
    console.error(❌ Extraction failed: , error.message);
    // Log failed extraction;
    await logExtractionJob({ sourceTitle: req.body.sourceTitle, error: error.message},;
      null,;
      "failed",;
      error.message);
 ...
 */;
app.post(/extract-batch, async (req: Request, res: 'Response) => {try {;
    const { chunks, sourceUrl, sourceTitle, context} = req.body;
    if (!Array.isArray(chunks) || chunks.length === 0) {return res.status(400).json({ error: "chunks must be a non-empty array of strings"});
    };
    if (chunks.length > 20) {return res.status(400).json({ error: Maximum 20 chunks per batch request});
    };
    console.log(`🧠 Processing batch extraction: ${chunks.length} chunks`);
    const results = [];
    let totalNodes = 0;
    let totalEdges = 0;
    for (let i = 0 i < chunks.length i++) {try {;
        const _prompt = buildExtractionPrompt(chunks[i], sourceUrl, `${sourceTitle} (chunk ${i+1})`, context);
        const _aiResponse = await callThinkingEngine('prompt');
        const parsed = parseAIResponse(aiResponse);
        results.push({chunk_index: i,;
          success: true,;
          extracted: parsed});
        totalNodes += parsed.nodes.length;
        totalEdges += parsed.edges.length} catch(chunkError: unknown) {results.push({;
          chunk_index: i,;
          success: false,;
  ...
  } catch(error: unknown) {console.error("❌ Batch extraction failed:", error.message);
    res.status(500).json({;
...
// ========================================;
app.use((error: Error, req: Request, res: Response, next: NextFunction) => {console.error("Unhandled error:", error);
  res.status(500).json({;
    error: Internal server error,;
    message: error.message,;
    timestamp: new Date().toISOString()})});
const server = app.listen(config.server.port, () => {console.log("🧠 Kaitiaki Aronui Cortex - ONLINE");
