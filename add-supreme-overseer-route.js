...
try {
  ...
  if (!content.includes('SupremeOverseerCommandCenter')) {
    ...
    if (importMatch) {
      const importStatement = 'import SupremeOverseerCommandCenter from \'./components/SupremeOverseerCommandCenter\';\n';
  ...
  if (!content.includes('/supreme-overseer')) {
    ...
    if (routeMatch) {
      const routeStatement = '        <Route path="/supreme-overseer" element={<SupremeOverseerCommandCenter />} />\n';
