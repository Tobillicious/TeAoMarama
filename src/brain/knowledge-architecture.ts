 ...
 * 
 * Philosophy: * - Every piece of knowledge has whakapapa (lineage)
...
export type KnowledgeType = | 'nzc_outcome'
  | 'curriculum_strand' 
  | 'year_plan'
  | 'term_plan'
  | 'unit_plan'
...
export type KnowledgeLevel = | 'meta'        // System-level insights, pedagogical patterns
...
export type RelationshipType = | 'contains'      // Year plan contains unit plans
...
export interface QueryStrategy {// Fast path - for immediate teacher needs,
hotCache: {,
 ...
 */
export interface MigrationIntelligence {// Phase 1: Extract and understand existing knowledge,
extract: (_source: TeKeteAkoSource) => Promise<RawKnowledgeChunk[]>
  
  // Phase 2: Classify and enrich with metadata,
classify: (_chunks: RawKnowledgeChunk[]) => Promise<ClassifiedNode[]>
  
  // Phase 3: Build relationships and detect patterns,
relate: (_nodes: ClassifiedNode[]) => Promise<KnowledgeNode[]>
  
  // Phase 4: Consolidate duplicates and improve quality,
consolidate: (_nodes: KnowledgeNode[]) => Promise<KnowledgeNode[]>
  
  // Phase 5: Cultural safety review and approval,
review: (_nodes: KnowledgeNode[]) => Promise<KnowledgeNode[]>
  
  // Phase 6: Index for optimal query performance,
index: (_nodes: KnowledgeNode[]) => Promise<QueryStrategy>}
