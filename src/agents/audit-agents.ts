...
Check for: 1. Tailwind CSS conflicts with regular CSS
...
return {,
issues: (response as unknown)?.issues || [],;,
suggestions: (response as unknown)?.suggestions || []
      }
    } catch {
      // Fallback parsing
return {,
...
Check for: 1. Accessibility (ARIA labels, keyboard nav)
...
Look for: 1. Large bundle imports 
...
COMPONENTS:
${componentFiles.map(f => `${f.path}:\n${f.content}`).join('\n\n')}
Check for: 1. Missing alt text on images
...
auditResults.forEach(_(result,  _index) => {
...
const overallScore = results.reduce(_(sum,  r) => sum + r.score, 0) / results.length
...
const totalIssues = results.reduce(_(sum,  r) => sum + r.issues.length, 0)
 ...
 */
export async function runQuickAudit(__files: { path: string content: string }[]): Promise<void> {
