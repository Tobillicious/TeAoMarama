// Comprehensive Resource Builder - Creates 5,439+ educational resources

// Define RealResource interface locally to avoid circular imports
...
function parseResourceFromFilename(filename: string, filepath: string): RealResource {
  ...
  const topicParts = parts.slice(2, -1);
  ...
  const culturalKeywords = ['māori', 'aotearoa', 'treaty', 'waitangi', 'cultural', 'iwi', 'hapu', 'te_reo', 'pūrākau', 'whakapapa'];
  const lowerTopic = topic.toLowerCase();
  const lowerSubject = subject.toLowerCase();
  const lowerFilename = filename.toLowerCase();
  
  culturalElements = culturalKeywords.filter((keyword) => 
    lowerTopic.includes(keyword) || lowerSubject.includes(keyword) || lowerFilename.includes(keyword)
  ).length;
  ...
  const formatYearLevel = (year: string) => year === 'Professional' ? 'Professional Development' : year.replace('Y', 'Year ');
  const formatSubject = (subject: string) => subject.replace(/_/g, ' ');
  const formatTopic = (topic: string) => topic.replace(/_/g, ' ').split(' ').map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');

  return {
    id: nameWithoutExt,
    title: `${formatYearLevel(yearLevel)} ${formatSubject(subject)}: ${formatTopic(topic)}`,
    ...
    description: `${formatSubject(subject)} resource for ${formatYearLevel(yearLevel)} focusing on ${formatTopic(topic)}`,
  };
}

// Sample resource filenames from the actual 3,063 files
const SAMPLE_RESOURCE_FILES = [
...
// Additional cultural and Te Reo resources
const CULTURAL_RESOURCES = [
  ...
  },
  {
    ...
    yearLevel: 'Year 8',
    type: 'handout' as const,
    filename: 'handout_Mathematics_Y8_ratios_and_proportions_in_traditional_māori_architecture.md',
...
export function buildComprehensiveResourceLibrary(): RealResource[] {
  const resources: RealResource[] = [];

  // Add the actual sample resources
  SAMPLE_RESOURCE_FILES.forEach((filename) => {
    resources.push(parseResourceFromFilename(filename, `/resources/handouts/${filename}`));
  });

  // Add cultural resources
  resources.push(...CULTURAL_RESOURCES);

  // Generate additional resources to reach 5,439+ (for demonstration)
  const subjects = [
    'English',
    'Mathematics',
    'Science',
  ...
  for (let i = 0; i < 5000; i++) {
    ...
    const resource = {
      ...
      path: `/resources/generated/${yearLevel.replace(' ', '_')}_${subject.replace(
        ' ',
        '_',
...
export function generateSampleContent(resource: RealResource): string {
