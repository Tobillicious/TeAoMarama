...
export class MockResourceGenerator {
  private static readonly SUBJECTS = [
    ...
    'Te Reo Māori', 'Technology', 'Health', 'Arts'
  ];
  ...
  private static readonly RESOURCE_TYPES = [
    'Handout', 'Activity', 'Video', 'Game', 'Slideshow', 'Link', 'Assessment'
  ];
  ...
  private static readonly CURRICULUM_AREAS = [
    ...
    'Identity Culture and Organisation', 'Place and Environment', 'Time Continuity and Change',
    'Health Promotion', 'Personal Health and Physical Development', 'Movement Concepts',
    'Visual Arts', 'Music', 'Drama', 'Dance'
  ];

  private static readonly CULTURAL_SAFETY_LEVELS: Array<'clean' | 'review' | 'consultation'> = 
    ['clean', 'clean', 'clean', 'clean', 'review', 'consultation']; // Weighted toward clean

  private static readonly SAMPLE_TITLES = [
    'Exploring Quadratic Functions', 'Chemical Reactions Lab', 'Narrative Writing Workshop',
    'New Zealand Democracy', 'Te Mauri o te Taiao', 'Digital Citizenship', 'Healthy Relationships',
    'Abstract Art Techniques', 'Algebraic Problem Solving', 'Ecosystem Interactions',
    'Poetry Analysis', 'World War I Impact', 'Conversational Māori', 'Programming Basics',
    'Sports Psychology', 'Music Composition', 'Data Collection Methods', 'Plant Biology',
    'Creative Writing', 'Historical Thinking', 'Cultural Protocols', 'Web Development',
    'Fitness Planning', 'Visual Design', 'Mathematical Modeling', 'Physics Experiments',
    'Literature Circles', 'Geography Skills', 'Māori Legends', 'Robotics Introduction',
    'Mental Health Awareness', 'Drama Performance'
  ];
  ...
  static generateMockResources(count: number): ParsedResource[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const metadata: ResourceMetadata = {
        ...
        culturalSafetyLevel,
        culturalSafetyIcon: culturalSafetyLevel === 'clean' ? '🟢' : 
                           culturalSafetyLevel === 'review' ? '🟡' : '🔴',
...
export class PerformanceTester {
  ...
  static async testSearchPerformance(resources: ParsedResource[]): Promise<number> {
    ...
    const searchQueries = [
      'math', 'science year 9', 'cultural activities', 
      'assessment tools', 'interactive digital resources'
    ];
  ...
  static async testFilterPerformance(resources: ParsedResource[]): Promise<number> {
    ...
    const filterTests = [
      { name: 'Subject Filter', fn: () => resources.filter(r => r.metadata.subject === 'Mathematics') },
      { name: 'Year Level Filter', fn: () => resources.filter(r => r.metadata.yearLevel === 'Year 9') },
      { name: 'Cultural Safety Filter', fn: () => resources.filter(r => r.metadata.culturalSafetyLevel === 'clean') },
      { name: 'Multiple Filters', fn: () => resources.filter(r => 
  ...
  static generateRecommendations(metrics: PerformanceMetrics): string[] {
    ...
    if (metrics.initialLoadTime > 3000) {
      recommendations.push('⚠️ Initial load time exceeds 3s - implement code splitting and lazy loading');
    }
    
    if (metrics.searchResponseTime > 100) {
      recommendations.push('⚠️ Search response time > 100ms - consider implementing debounced search and indexing');
    }
    
    if (metrics.filterResponseTime > 50) {
      recommendations.push('⚠️ Filter response time > 50ms - optimize filter algorithms and consider memoization');
    }
    
    if (metrics.memoryUsage.peak > 100) {
      recommendations.push('⚠️ Memory usage exceeds 100MB - implement virtualization for large lists');
    }
    
    if (metrics.scrollPerformance.fps < 55) {
      recommendations.push('⚠️ Scroll FPS below 55 - implement virtual scrolling and optimize rendering');
    }
    
    if (metrics.scrollPerformance.jankCount > 5) {
      recommendations.push('⚠️ High jank count detected - optimize heavy computations and rendering');
    }
