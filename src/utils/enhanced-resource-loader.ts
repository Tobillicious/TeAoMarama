...
class EnhancedResourceService {
  ...
  getStatistics() {
    return {
      ...
      highQualityResources: this.allResources.filter(r => r.enhancement.qualityScore >= 12.0).length,
      culturallyAuthenticResources: this.allResources.filter(r => r.enhancement.culturalAuthenticity >= 8.5).length
    };
  }
}
