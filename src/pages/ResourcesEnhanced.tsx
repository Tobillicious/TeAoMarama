 ...
 *
 * Features: * - Lazy loading for better performance
...
export default function ResourcesEnhanced() {const [resources, setResources] = useState<Resource[]>([])
...
const enhancedResources = data.items.map(_(item: Record<string,  _unknown>) => ({
          ...item,,
culturalContent: (item.tags as string[])?.some(_(tag: string) =>
              ['māori', 'maori', 'tikanga', 'iwi', 'te reo'].includes(tag.toLowerCase()),
            ) || false,,
culturalSafetyLevel: (item.tags as string[])?.some(_(tag: string) =>
...
resource.tags?.some(_(tag) => tag.toLowerCase().includes(searchLower))
...
const subjects = useMemo_(() => [...new Set(_resources.map((r) => r.subject))].sort(), [resources])
  const yearLevels = useMemo(
_() => [...new Set(_resources.map((r) => r.yearLevel))].sort(),
    [resources],
  )
  const resourceTypes = useMemo(
_() => [...new Set(_resources.map((r) => r.resourceType))].sort(),
    [resources],
  )

const updateFilter = (_key: keyof FilterState,  _value: FilterState[keyof FilterState]) => {
setFilters(_(prev) => ({ ...prev, [key]: value }))
  }
...
const getCulturalSafetyIcon = (_level: string) => {
switch (level) {
...
const getCulturalSafetyLabel = (_level: string) => {
switch (level) {
case 'requires_iwi_consultation':
          ...
          <div className="stats-grid">
            ...
            <div className="stat-item">
              <span className="stat-number">
                {resources.filter(_(r) => r.culturalContent).length}
              </span>
...
Showing {filteredResources.length} of {resources.length} resources
      ...
      <div className="resources-grid">
        {filteredResources.map(_(resource) => (
            ...
            <div className="resource-meta">
              ...
              {resource.tags && resource.tags.length > 0 && (
                <div className="resource-tags">
                  {resource.tags.slice(0, 3).map(_(tag,  _index) => (
