...
export default function GovernmentComponentAnalysis() {const [currentPage, setCurrentPage] = useState(1)
...
const updateSystemData = (_systemKey: 'system1' | 'system2',  _field: string,  _value: string) => {
setAnalysisData(_(prev) => ({
      ...prev,
      [systemKey]: {
        ...prev[systemKey],
        [field]: value,
...
const updateComparison = (_field: string,  _value: string) => {
setAnalysisData(_(prev) => ({
      ...prev,;,
comparison: {
        ...prev.comparison,
...
const updateCulturalInsights = (_field: string,  _value: string) => {
setAnalysisData(_(prev) => ({
      ...prev,;,
culturalInsights: {
...
const updateSocietyApplication = (_field: string,  _value: string) => {
setAnalysisData(_(prev) => ({
      ...prev,;,
societyApplication: {
...
_<div className="page-navigation">
      {[1,  _2,  _3,  _4].map((pageNum) => (
_<button
key={pageNum}
...
const renderPage1 = () => (
_<div className="worksheet-page">
...
onChange={(e) => setStudentInfo(_(prev) => ({ ...prev, groupNames: e.target.value }))}
...
onChange={(_e) => setStudentInfo(_(prev) => ({ ...prev, date: e.target.value }))}
            />
          </div>
          <div className="info-field">
            <label>Period:</label>
...
onChange={(_e) => setStudentInfo(_(prev) => ({ ...prev, period: e.target.value }))}
...
contrasting systems to identify their strengths, challenges, and key components.
      ...
      <div className="analysis-section">
        ...
        <div className="component-grid">
          {governmentComponents.map(_(component) => (
...
onChange={(_e) => updateSystemData('system1', 'name', e.target.value)}
            />
            <input
type="text"
placeholder="Where/When"
value={analysisData.system1.location || ''}
onChange={(_e) => updateSystemData('system1', 'location', e.target.value)}
            />
            <textarea
placeholder="Why this system interests your group: "
value={analysisData.system1.uniqueFeatures?.[0] || ''}
onChange={(_e) => updateSystemData('system1', 'uniqueFeatures', [e.target.value])}
rows={3}
            />
          </div>
        </div>
...
onChange={(_e) => updateSystemData('system2', 'name', e.target.value)}
            />
            <input
type="text"
placeholder="Where/When"
value={analysisData.system2.location || ''}
onChange={(_e) => updateSystemData('system2', 'location', e.target.value)}
            />
            <textarea
placeholder="How this system differs from System 1: "
value={analysisData.system2.uniqueFeatures?.[0] || ''}
onChange={(_e) => updateSystemData('system2', 'uniqueFeatures', [e.target.value])}
rows={3}
            />
          </div>
        </div>
...
const renderSystemAnalysisPage = (_systemKey: 'system1' | 'system2',  _pageNumber: number) => {
...
onChange={(_e) => updateSystemComponent(systemKey, component.id, e.target.value)}
rows={3}
                />
              </div>
            ))}
...
onChange={(_e) => updateComparison('sharedStrengths', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateComparison('commonProblems', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateComparison('preferredSystem', e.target.value)}
rows={4}
            />
          </div>
        </div>
...
onChange={(_e) => updateCulturalInsights('culturalInfluence', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateCulturalInsights('indigenousLearnings', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateCulturalInsights('maoriPrinciples', e.target.value)}
rows={4}
            />
          </div>
        </div>
...
onChange={(_e) => updateSocietyApplication('authorityStructure', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateSocietyApplication('decisionProcess', e.target.value)}
rows={4}
            />
          </div>
...
onChange={(_e) => updateSocietyApplication('uniqueFeatures', e.target.value)}
rows={4}
            />
          </div>
        </div>
...
onChange={(_e) =>
setAnalysisData(_(prev) => ({ ...prev, unifiedPurposeReflection: e.target.value }))
            }
rows={4}
          />
        </div>
