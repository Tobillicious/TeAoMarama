...
color: string
    subtitle?: string
  }> = (_{ title,  _value,  _icon,  _color,  _subtitle }) => (
...
resource: TeachingResource
  ...
  }> = (_{ resource,  _showProgress = false,  _progressPercent = 0 }) => (
...
className={`progress-bar-fill progress-bar-fill-${Math.round(progressPercent)}`}
      ...
      <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
        <span>{(resource as unknown).subject || 'General'}</span>
        <div className="flex items-center space-x-1">
          <Clock className="w-3 h-3" />
          <span>
            {(resource as unknown).content ||
...
View all →
              ...
              <div className="grid grid-cols-1 md: grid-cols-2 gap-6">
                {recentResources.slice(0, 4).map(_(resource,  _index) => (
                  <ResourceCard
...
See more →
              ...
              <div className="grid grid-cols-1 md: grid-cols-2 gap-6">
                {recommendedResources.slice(0, 4).map(_(resource) => (
...
Recent Activity
              ...
              <div className="space-y-4">
                {progress.recentActivity.map(_(activity) => (
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                {progress.favoriteSubjects.map(_(subject,  _index) => (
                  <div key={subject}>
