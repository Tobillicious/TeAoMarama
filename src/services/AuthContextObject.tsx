...
export interface AuthContextType {
  ...
  updateUserProfile: (updates: Partial<UserProfile>) => Promise<ProfileUpdateResult>;
  ...
  canAccessCulturalContent: (level: 'low' | 'medium' | 'high' | 'critical') => boolean;
}

export const AuthContext = createContext<AuthContextType | undefined>(undefined);

// Type guards for better type safety
export const isTeacher = (profile: UserProfile | null): profile is UserProfile & { role: 'teacher' } => {
  return profile?.role === 'teacher';
};

export const isStudent = (profile: UserProfile | null): profile is UserProfile & { role: 'student' } => {
  return profile?.role === 'student';
};

export const isAdmin = (profile: UserProfile | null): profile is UserProfile & { role: 'admin' } => {
  return profile?.role === 'admin';
};

export const hasRole = (profile: UserProfile | null, role: UserProfile['role']): boolean => {
  return profile?.role === role;
};

// Cultural sensitivity helpers
export const getCulturalGreeting = (culturalContext: CulturalContext, timeOfDay: 'morning' | 'afternoon' | 'evening'): string => {
...
export const getErrorMessageInPreferredLanguage = (
  errorKey: string, 
  culturalContext: CulturalContext
): string => {
  const errorMessages = {
