 ...
 * 
 * This service ensures every resource meets the highest standards for: * - New Zealand Curriculum alignment  
...
Subject: ${(resource as unknown).subject || 'General'},
Description: ${resource.description}
Year Levels: ${resource.yearLevel.join(', ')}
Provide detailed NZC alignment including: 1. Specific curriculum level and learning objectives
...
learningArea: this.mapSubjectToLearningArea((resource as unknown).subject || 'General'),;,
strand: this.inferStrand((resource as unknown).subject || 'General', resource.type),;,
...
Resource: ${resource.__title || resource.id || 'Unknown'} (${(resource as unknown).subject || 'General'})
Current Cultural Content: ${resource.culturalContent.hasMaoriContent ? 'Contains Māori content' : 'No specific cultural content'}
Provide culturally responsive integration including: 1. Treaty of Waitangi principles (Partnership, Participation, Protection)
...
gradingRubric: `${(resource as unknown).subject || 'General'}_${resource.type}_rubric`,,
reportingCategory: this.mapToKamarCategory((resource as unknown).subject || 'General'),;,
trackingCodes: [`NZC_${(resource as unknown).subject || 'General'.replace(' ', '_').toUpperCase()}`],;,
...
Resource: ${resource.__title || resource.id || 'Unknown'} (${(resource as unknown).subject || 'General'}),
Description: ${resource.description}
Create authentic connections to: 1. Other learning areas in NZC
...
Ensure connections are: - Meaningful and natural (not forced)
    - Educationally sound
...
return {,
primarySubject: (resource as unknown).subject || 'General',,
connections: [
        {,
...
const yearNumbers = yearLevels.map(y => parseInt(y.replace('Year ', '')))
    const avgYear = yearNumbers.reduce(_(a,  _b) => a + b, 0) / yearNumbers.length
...
return `engage with and understand ${(resource as unknown).subject || 'General'.toLowerCase()} concepts through ${resource.type.replace('_', ' ')} activities`
  }
...
export const _aotearoaEducationService = new AotearoaEducationService()