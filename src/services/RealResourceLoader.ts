...
export class RealResourceLoader {
  ...
  private async loadMigratedResources(): Promise<void> {
    try {
      ...
      this.resourceCache = sampleResources.map((resource) =>
    ...
    const realMigratedData: MigratedResource[] = [
      {
        ...
        textBody:
          "In this comprehensive lesson, students will identify, describe, and extend number patterns using visual grids. They'll develop algebraic thinking skills while exploring mathematical relationships in engaging, hands-on activities that connect to real-world applications.",
      ...
      },
      {
        ...
        summary:
          'Comprehensive exploration of the Treaty of Waitangi and its ongoing significance.',
        textBody:
      ...
      },
      {
        ...
        summary:
          'Explore nutrition concepts with focus on New Zealand dietary patterns and health.',
        textBody:
  ...
  private convertToTeachingResource(migrated: MigratedResource): TeachingResource {
    return {
      ...
      nzcAlignment: migrated.curriculumCandidates.map((c) => c.code),
      culturalContent: {
        hasMaoriContent:
          migrated.culturalContent ||
          migrated.suggestedTags.some((tag) =>
  ...
  private calculateRating(migrated: MigratedResource): number {
    ...
    if (migrated.curriculumCandidates.length > 0) {
      const avgConfidence =
        migrated.curriculumCandidates.reduce((sum, c) => sum + c.confidence, 0) /
  ...
  async searchResources(query: string): Promise<TeachingResource[]> {
    ...
    return allResources.filter(
      (resource) =>
        ...
        resource.nzcAlignment.some((code) => code.toLowerCase().includes(queryLower)),
    );
  }
  ...
  async getResourcesBySubject(subject: string): Promise<TeachingResource[]> {
    const allResources = await this.getAllResources();
    return allResources.filter((resource) => resource._____subject === subject);
  }
  ...
  getMigrationStats() {
    return {
      ...
      culturalContent: this.resourceCache.filter((r) => r.culturalContent.hasMaoriContent).length,
      subjects: [...new Set(this.resourceCache.map((r) => r._____subject))],
      yearLevels: [...new Set(this.resourceCache.flatMap((r) => r.yearLevel))],
