...
export class RealResourceLoader {
  ...
  private async loadMigratedResources(): Promise<void> {
    try {
      ...
      this.resourceCache = sampleResources.map(resource => this.convertToTeachingResource(resource));
  ...
  private convertToTeachingResource(migrated: MigratedResource): TeachingResource {
    return {
      ...
      culturalContent: {
        hasMaoriContent: migrated.culturalContent || migrated.suggestedTags.some(tag => 
  ...
  private generateId(url: string): string {
    return url.split('/').pop()?.replace(/[^a-zA-Z0-9]/g, '-') || 'resource-' + Math.random().toString(36).substr(2, 9);
  }
  ...
  private assessCulturalSensitivity(migrated: MigratedResource): 'low' | 'medium' | 'high' | 'critical' {
    if (migrated.manual_review) return 'high';
    if (migrated.culturalContent) return 'medium';
    if (migrated.suggestedTags.some(tag => ['te ao māori', 'treaty', 'cultural'].includes(tag.toLowerCase()))) {
      return 'medium';
    }
    return 'low';
  }
  ...
  private extractTikangaElements(migrated: MigratedResource): string[] {
    ...
    if (content.includes('treaty') || content.includes('tiriti')) elements.push('tiriti o waitangi');
  ...
  private calculateRating(migrated: MigratedResource): number {
    ...
    if (migrated.curriculumCandidates.length > 0) {
      const avgConfidence = migrated.curriculumCandidates.reduce((sum, c) => sum + c.confidence, 0) / migrated.curriculumCandidates.length;
  ...
  async getResourcesBySubject(subject: string): Promise<TeachingResource[]> {
    const allResources = await this.getAllResources();
    return allResources.filter(resource => resource.subject === subject);
  }
  ...
  getMigrationStats() {
    return {
      ...
      subjects: [...new Set(this.resourceCache.map(r => r.subject))],
