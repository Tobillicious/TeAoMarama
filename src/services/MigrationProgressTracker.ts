...
private progress: MigrationProgress
  ...
  private listeners: Array<(_progress: MigrationProgress) => void> = []
...
averageQualityScore: completed.length > 0 
        ? (_completed.reduce((sum,  r) => sum + r.qualityScore, 0) / completed.length).toFixed(2)
        : '0.00',,
averageProcessingTime: completed.length > 0
        ? (_completed.reduce((sum,  r) => sum + r.processingTime, 0) / completed.length).toFixed(0)
        : '0',,
...
return this.resourceStatuses.get(resourceId)
   ...
   */
subscribe(_listener: (progress: MigrationProgress) => void): () => void {
...
export const _migrationProgressTracker = new MigrationProgressTracker()
