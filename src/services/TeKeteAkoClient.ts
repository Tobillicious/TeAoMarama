...
function getEnv(__key: string): string | undefined {
...
if (sampleData) {
const culturalCount = sampleData.filter(_(item) =>
this.culturalKeywords.some(_(keyword) =>
...
_detectedKeywords.some((k) => ['tikanga', 'mātauranga', 'traditional'].includes(k))
          ) {
riskLevel = 'high'
            flagType = 'traditional_knowledge'
          } else if (
_detectedKeywords.some((k) => ['pacific', 'pasifika', 'samoa', 'tonga'].includes(k))
          ) {
riskLevel = 'medium'
...
await this.logActivity(_'cultural_content_scan',  _{,
table_scanned: tableName, ,
_items_scanned: data?.length || 0, ,
_cultural_flags_created: flags.length, ,
_high_risk_items: flags.filter(
