        ...
        <div key={action.id} className="relative">
          ...
          {showTooltip === action.id && (
            <div className="absolute top-full mt-2 left-0 bg-text text-white text-xs rounded-md px-2 py-1 whitespace-nowrap z-10">
              {action.reason}
              <div className="text-xs opacity-75">Confidence: {(action.confidence * 100).toFixed(0)}%</div>
            </div>
          )}
        </div>
      ))}
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {memory.recentPages.map((path, index) => (
            <Link
              key={path}
              to={path}
      ...
      <div>
        ...
        <div className="space-y-2">
          {memory.frequentPaths.map((item) => (
            <div key={item.path} className="flex items-center justify-between px-3 py-2 rounded-md bg-white">
