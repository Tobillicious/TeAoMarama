...
class KaitiakiAronuiOverseer {
   ...
   */
  async analyzeTeKeteAkoContent(): Promise<MigrationIntelligence> {
    console.log('🧠 Kaitiaki Aronui: Analyzing Te Kete Ako content with supreme intelligence...');
    ...
    try {
      ...
      try {
        ...
        intelligence.enhancedHandouts = enhancedFiles.length;
      } catch (err) {
        console.log('📝 No enhanced directory found, continuing with main handouts...');
      }
  ...
  private async analyzeHandoutsDirectory(dirPath: string, isEnhanced = false): Promise<any[]> {
    const handouts = [];
  ...
  private async migrateHandoutWithDeepSeek(handout: any): Promise<void> {
    try {
      ...
      const enhancedComponent = await this.generateReactComponent(
        htmlContent,
        componentName,
        handout
      );
      ...
      console.error(`❌ Migration failed for ${handout.title}:`, error);
    }
  }
  ...
  private async generateReactComponent(htmlContent: string, componentName: string, handout: any): Promise<string> {
    // For now, generate component without DeepSeek API call to avoid rate limits
...
const ${componentName}: React.FC<${componentName}Props> = ({
  culturalContext = "${handout.culturalAuthenticity >= 80 ? 'High cultural authenticity content' : 'Educational content with cultural integration'}",
...
`;
  ...
  private extractMainContent(htmlContent: string): string {
    ...
    if (articleMatch) {
      return this.cleanHtmlForReact(articleMatch[1]);
    }
    
    // Fallback to extracting content between common markers
    const bodyMatch = htmlContent.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
    if (bodyMatch) {
      return this.cleanHtmlForReact(bodyMatch[1]);
    }
    
    return '<p>Content extraction in progress...</p>';
  }

  private cleanHtmlForReact(html: string): string {
    return html
...
if (import.meta.main) {
  const kaitiaki = new KaitiakiAronuiOverseer();
  
  kaitiaki.analyzeTeKeteAkoContent()
    .then(intelligence => {
      console.log('📊 MIGRATION INTELLIGENCE REPORT:');
      intelligence.migrationStrategy.forEach(line => console.log(line));
      
      return kaitiaki.executeAutomatedMigration(intelligence, 3);
    })
    .then(() => {
      console.log('🌟 Kaitiaki Aronui migration batch complete!');
    })
    .catch(error => {
      console.error('❌ Kaitiaki Aronui encountered an error:', error);
    });
}
