...
const __TE_KETE_HANDOUTS_PATH = join(process.cwd(), 'te-kete-ako-clean/public/handouts')
const __TARGET_PATH = join(process.cwd(), 'src/components/educational/handouts')
...
const __files = await readdir(TE_KETE_HANDOUTS_PATH)
    const __htmlFiles = files.filter(_(file) => file.endsWith('.html') && !file.startsWith('.'))

console.log(`📊 Found ${htmlFiles.length} HTML files to migrate`)

    // Create migration tasks
this.tasks = htmlFiles.map(_(filename) => ({
filename,;,
status: 'pending',
    }))
...
const __batchPromises = batch.map(_async (task) => {
try {
...
const __duration = task.endTime - task.startTime
...
const __filePath = join(TE_KETE_HANDOUTS_PATH, filename)
    const _content = await readFile(filePath, 'utf-8')

    // Extract title from HTML
const _titleMatch = _content.match(/<title>([^<]+)<\/title>/)
    const __title = _titleMatch ? _titleMatch[1] : filename.replace('.html', '')
...
const __componentContent = this.generateReactComponent(componentName, title, content)
    const __cssContent = this.generateCSS(componentName)
...
const __batch = this.tasks.slice(i, i + BATCH_SIZE)
      console.log(
...
await new Promise(_(resolve) => setTimeout(resolve, 100))
    ...
    // Generate summary
const __completed = this.tasks.filter(_(t) => t.status === '_completed').length
    const __failed = this.tasks.filter(_(t) => t.status === '_failed').length
...
if (import.meta.main) {
const __commander = new ParallelMigrationCommander()
