...
if (extname(file) === '.tsx') {
const __originalName = basename(file, '.tsx')
          const __componentName = this.sanitizeComponentName(originalName)
          const __importPath = `./components/educational/handouts/${originalName}`
          const __routePath = `/${originalName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
...
const __originalName = basename(file, '.tsx')
          const __componentName = this.sanitizeComponentName(originalName)
          const __importPath = `./pages/${originalName}`
          const __routePath = `/${originalName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
...
return name
      .replace(/[^a-zA-Z0-9]/g, '')
      .replace(_/^[a-z]/,  _(c) => c.toUpperCase())
...
const __routeManager = new SmartRouteManager()
routeManager
  .addMissingRoutes()
  .then(() => {
