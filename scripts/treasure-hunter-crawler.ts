...
class TreasureHunterCrawler {
  ...
  private isTreasureFile(filePath: string): boolean {
    ...
    if (name.includes('maori') || name.includes('cultural') || name.includes('tikanga') || 
        name.includes('whakatauki') || name.includes('purakau') || name.includes('waiata')) {
      return true;
    }
    
    // Assessment and educational files
    if (name.includes('assessment') || name.includes('lesson') || name.includes('unit') ||
        name.includes('handout') || name.includes('activity') || name.includes('worksheet')) {
      return true;
    }
  ...
  private determineTreasureType(filePath: string, content: string): Treasure['type'] {
    ...
    // Cultural treasures
    if (path.includes('maori') || path.includes('cultural') || path.includes('tikanga') ||
        contentLower.includes('whakatauki') || contentLower.includes('purakau') ||
        contentLower.includes('waiata') || contentLower.includes('karakia')) {
      return 'cultural';
    }
    
    // Assessment treasures
    if (path.includes('assessment') || path.includes('rubric') || path.includes('evaluation') ||
        contentLower.includes('assessment') || contentLower.includes('rubric')) {
      return 'assessment';
    }
    
    // Historical treasures
    if (path.includes('historical') || path.includes('history') || path.includes('legacy') ||
        contentLower.includes('historical') || contentLower.includes('history')) {
      return 'historical';
    }
    
    // Technical treasures
    if (path.includes('technical') || path.includes('script') || path.includes('config') ||
        contentLower.includes('technical') || contentLower.includes('script')) {
      return 'technical';
    }
  ...
  private calculateCulturalRelevance(content: string): number {
    ...
    const culturalKeywords = [
      'whakatauki', 'purakau', 'tikanga', 'kaitiakitanga', 'mana', 'tapu', 'noa',
      'maori', 'te reo', 'waiata', 'karakia', 'haka', 'marae', 'whanau', 'iwi',
      'hapu', 'rangatiratanga', 'kawanatanga', 'tino rangatiratanga'
    ];
...
treasureHunter
  ...
  .then((treasureMap) => {
    ...
    treasureMap.treasures.slice(0, 10).forEach((treasure, index) => {
      console.log(`${index + 1}. ${treasure.name} (${treasure.value}/100) - ${treasure.description}`);
    });
    ...
    writeFileSync('reports/treasure-summary.md', `
...
## Top Treasures
${treasureMap.treasures.slice(0, 10).map((t, i) => `${i + 1}. **${t.name}** (${t.value}/100) - ${t.description}`).join('\n')}
