...
CRITICAL CULTURAL SAFETY PROTOCOLS: - Always include cultural context appropriately
...
try {
const __response = await axios.post(
...
const __prompt = `Create a DEPTH-FOCUSED ${type} for ${subject} Year ${yearLevel} on the topic: ${topic},
...
Requirements: - DEPTH: Progressive complexity building on solid foundations
...
Include: - "Previous Learning Connections" section
...
try {
const __content = await this.generateContent(prompt, culturalContext)
      
      // Create directory if it doesn't exist
const __baseDir = join(process.cwd(), 'migration', 'recovered_resources', 'deepseek-generated')
      if (!existsSync(baseDir)) {
mkdirSync(baseDir, { recursive: true })
      }
const __filename = `${type}_${subject}_Y${yearLevel}_${topic.replace(/\s+/g, '_').toLowerCase()}.md`
      const __filepath = join(baseDir, filename)
...
// Main execution
async function main() {const __agent = new DeepSeekAgent()
