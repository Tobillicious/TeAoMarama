...
const _teKeteFiles = await readdir(_teKetePath)
      const _teKeteHandouts = _teKeteFiles.filter(_(f) => f.endsWith('.html') && !f.startsWith('.'))

const _migratedFiles = await readdir(_targetPath)
      const _migratedComponents = _migratedFiles.filter(_(f) => f.endsWith('.tsx'))
...
const _files = await readdir(dir)
        const _tsxFiles = _files.filter(_(f) => f.endsWith('.tsx'))
        const _cssFiles = _files.filter(_(f) => f.endsWith('.css'))
...
missingDocs.forEach(_(doc) => console.log(`    - ${doc}`))
    }
...
outdatedContext.forEach(_(file) => console.log(`    - ${file}`))
    }
...
const _totalIssues = results.reduce(_(sum,  result) => sum + result.count, 0)

results.forEach(_(result) => {
const _status = result.count === 0 ? '✅' : '⚠️'
      console.log(`${_status} ${result.category}: ${result.count} issues`)
      if (result.issues.length > 0) {
result.issues.forEach(_(issue) => console.log(`    - ${issue}`))
      }
    })
