#!/usr/bin/env tsx
/**
 * 🛣️ ADD ALL MISSING ROUTES SCRIPT
 *
 * Adds routes for ALL the orphaned components that were migrated from Te Kete Ako
 * This script will add all 299 missing routes to App.tsx
 */
...
importPath: string,
routePath: string}
class CompleteRouteManager {
private componentsDir = 'src/components/educational/handouts'
  private pagesDir = 'src/pages'
  private appTsxPath = 'src/App.tsx'

async addAllMissingRoutes(): Promise<void> {
console.log('🛣️ Adding ALL missing routes for orphaned components...\n')

    // Get all component files
const __componentFiles = await this.getAllComponentFiles()
    const __existingRoutes = await this.getExistingRoutes()

    // Find missing routes
const __missingRoutes = componentFiles.filter(
_(comp) => !existingRoutes.includes(comp.componentName),
    )
...
const __componentName = basename(file, '.tsx')
          const __importPath = `./components/educational/handouts/${componentName}`
          const __routePath = `/${componentName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
...
if (extname(file) === '.tsx') {
const __componentName = basename(file, '.tsx')
          const __importPath = `./pages/${componentName}`
          const __routePath = `/${componentName.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
...
try {
const __appContent = await readFile(this.appTsxPath, 'utf8')
      const existingRoutes: string[] = []

      // Extract component names from lazy imports
const __lazyImportRegex = /const (\w+) = lazy(/g
      let match
...
const __lazyImports = missingRoutes
        .map(_(route) => `const ${route.componentName} = lazy(() => import('${route.importPath}'))`)
        .join('\n')

      // Find the position to insert new imports (after existing lazy imports)
const __importEndIndex = appContent.lastIndexOf(
        '// Newly migrated components from Te Kete Ako',
      )
      if (importEndIndex !== -1) {
const __insertPosition = appContent.indexOf('\n', importEndIndex) + 1
        appContent =
...
const __spinnerIndex = appContent.indexOf('// Optimized loading component')
        if (spinnerIndex !== -1) {
appContent =
...
const __routeEntries = missingRoutes
        .map(_(route) => `      { path: '${route.routePath}', element: <${route.componentName} /> },`)
        .join('\n')

      // Find the routes array and add new routes
const __routesEndIndex = appContent.lastIndexOf('    ],')
      if (routesEndIndex !== -1) {
appContent =
...
await writeFile(this.appTsxPath, appContent, 'utf8')
      console.log('✅ Updated App.tsx with all missing routes')
    } catch (error) {
...
// Run the complete route manager
const __routeManager = new CompleteRouteManager()
routeManager
  .addAllMissingRoutes()
  .then(() => {
console.log('\n🛣️ Complete route addition finished!')
    process.exit(0)
  })
  .catch(_(error) => {
