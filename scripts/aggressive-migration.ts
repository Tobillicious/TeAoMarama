...
async function aggressiveMigrate(___filePath: string): Promise<void> {
try {
...
async function main() {console.log('🚀 AGGRESSIVE MIGRATION STARTED')
  
await mkdir(TARGET_PATH, { recursive: true})
  
const __files = await readdir(TE_KETE_HANDOUTS_PATH)
  const __handouts = files.filter(f => f.endsWith('.html') && !f.startsWith('.')).slice(0, 50)
