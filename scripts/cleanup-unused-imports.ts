...
line: number,
used: boolean}
async function cleanupUnusedImports() {console.log('🧹 CLEANING UP UNUSED IMPORTS IN APP.TSX...\n')

try {
    // Read App.tsx
const __appContent = await readFile('src/App.tsx', 'utf8')
    const __lines = appContent.split('\n')
...
const __match = line._match(importRegex)
      if (match) {
imports.push({,
name: match[1],,
path: match[2],,
...
const __routeSection = appContent.substring(appContent.indexOf('<Routes>'))
    const __usedImports = new Set<string>()

imports.forEach(_(importInfo) => {
...
const __unusedImports = imports.filter(_(imp) => !imp.used)
    const __usedImportsCount = imports.filter(_(imp) => imp.used).length
...
unusedImports.forEach(_(importInfo) => {
console.log(`🗑️  Removing unused import: ${importInfo.name}`)

      // Find the line to remove
const __lines = newContent.split('\n')
      const __lineIndex = importInfo.line - 1 - offset
