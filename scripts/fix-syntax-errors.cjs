...
function fixSyntaxErrors(filePath) {
  try {
    ...
    content = content
      ...
      // Fix object property assignments with extra semicolons
      .replace(/(\w+):\s*'([^']*)',;/g, "$1: '$2',")
      .replace(/(\w+):\s*"([^"]*)",;/g, '$1: "$2",')
      .replace(/(\w+):\s*(\d+),;/g, '$1: $2,')
      .replace(/(\w+):\s*(true|false),;/g, '$1: $2,')
      .replace(/(\w+):\s*(null|undefined),;/g, '$1: $2,')
      ...
      // Fix function parameters with extra semicolons
      .replace(/\(([^)]*)\),;/g, '($1),')
      
      // Fix method calls with extra semicolons
      .replace(/(\w+)\(([^)]*)\),;/g, '$1($2),')
      
      // Fix template literals with extra semicolons
      .replace(/`([^`]*)`,;/g, '`$1`,')
