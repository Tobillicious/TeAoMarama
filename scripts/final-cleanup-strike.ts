...
try {
const __issuesFixed = await this.fixTypeIssuesInFile(file)
        this.totalIssuesFixed += issuesFixed
        if (issuesFixed > 0) {
...
newContent = newContent.replace(/miharaStatus\.([a-zA-Z]+)/g, '(miharaStatus as unknown).$1')
...
const __finalStrike = new FinalCleanupStrike()
