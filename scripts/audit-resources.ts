...
async function auditResources(): Promise<void> {
  ...
  try {
    ...
    if (orphanedComponents.length > 0) {
      ...
      orphanedComponents.slice(0, 10).forEach((c) => console.log(`  - ${c}`));
    ...
    if (componentsWithoutCSS.length > 0) {
      ...
      componentsWithoutCSS.slice(0, 10).forEach((c) => console.log(`  - ${c}`));
    ...
    if (orphanedCSS.length > 0) {
      ...
      orphanedCSS.slice(0, 10).forEach((c) => console.log(`  - ${c}`));
