...
function cleanupAppTsx() {console.log('🧹 Cleaning up App.tsx...')

  // Read the current file
let content = fs.readFileSync(appTsxPath, 'utf8')

  // Extract all unique component imports
const __importRegex = /const\s+([a-zA-Z0-9_]+)\s*=\s*lazy(()\s*=>\s*import('([^']+)'))/g
  const __imports = new Map()

let match
  while ((match = importRegex.exec(content)) !== null) {
const __variableName = match[1]
    const __importPath = match[2]
...
const __routeRegex = /\{\s*path: \s*'([^']+)',\s*element: \s*<([^>]+)\s*/>\s*\}/g
  const __routes = new Map()

while ((match = routeRegex.exec(content)) !== null) {
const __path = match[1]
    const __component = match[2]
...
newContent += `const ${variableName} = lazy_(() => import('${importPath}'))\n`
  }
newContent += `}
function App() {return (
    <Router>
