...
files: ['src/services/RealResourceLoader.ts', 'src/services/ResourceService.ts'],
  ...
  // Unknown type issues
  {,
pattern: '\(_s: unknown\) => s\\.id',;,
replacement: '(_s: unknown) => s.id',;,
...
const __realResourceLoaderPath = 'src/services/RealResourceLoader.ts'
  if (fs.existsSync(realResourceLoaderPath)) {
const __content = fs.readFileSync(realResourceLoaderPath, 'utf8')
...
const __resourceServicePath = 'src/services/ResourceService.ts'
  if (fs.existsSync(resourceServicePath)) {
const __content = fs.readFileSync(resourceServicePath, 'utf8')
    const __newContent = content.replace(
