...
const __stageDuration = Date.now() - stageStart
...
setTimeout_(() => reject(new Error(`Command timeout: ${command}`)), timeout)
        )
...
const __complianceCheck = await execAsync(
...
const __duration = Date.now() - this.startTime.getTime()
...
const __timestamp = new Date().toISOString()
    const __logEntry = `[${timestamp}] ${message}\n`
...
const __pipeline = new ArmyDeploymentPipeline()
