...
content: string
  ...
  yearLevel?: string
  subject?: string}
const __TE_KETE_HANDOUTS_PATH = '/Users/admin/gemini-react-app/te-kete-ako-clean/public/handouts/'
const __TARGET_PATH = '/Users/admin/gemini-react-app/src/components/educational/handouts/'

async function extractHandoutMetadata(___htmlContent: string): Promise<Partial<HandoutData>> {
  // Extract title from h1 or h2 tags
const __titleMatch = htmlContent.match(/<h[12][^>]*>([^<]+)</h[12]>/i)
  const __title = titleMatch ? titleMatch[1].trim() : ''

  // Extract year level from content
const __yearMatch = htmlContent.match(/Year?\s*(\d+)|Y(\d+)/i)
  const __yearLevel = yearMatch ? `Year ${yearMatch[1] || yearMatch[2]}` : undefined
...
const __culturalContext = htmlContent.includes('māori') || htmlContent.includes('cultural') || htmlContent.includes('traditional')
...
const __componentName = handout.filename
    .replace(/\.html$/, '')
    .replace(/[-_]/g, ' ')
...
async function migrateHandout(___filename: string): Promise<void> {
try {
const __htmlPath = join(TE_KETE_HANDOUTS_PATH, filename)
    const __htmlContent = await readFile(htmlPath, 'utf-8')
    
    // Extract body content, removing HTML document structure
const __bodyMatch = htmlContent.match(/<body[^>]*>([\s\S]*?)</body>/i)
    const __content = bodyMatch ? bodyMatch[1].trim() : htmlContent
    
const __metadata = await extractHandoutMetadata(htmlContent)
...
const __reactComponent = generateReactComponent(handout)
    const __componentFilename = filename.replace(/\.html$/, '.tsx')
    const __outputPath = join(TARGET_PATH, componentFilename)
...
const __files = await readdir(TE_KETE_HANDOUTS_PATH)
    const __handoutFiles = files.filter(file => file.endsWith('.html') && !file.startsWith('.'))
...
const __batchSize = Math.max(1, Math.ceil(handoutFiles.length / 40))
...
await Promise.all(
_batches.map(async (batch,  _index) => {
console.log(`🔄 Agent batch ${index + 1} processing ${batch.length} files`)
    ...
    // Generate index file
const __successfulMigrations = handoutFiles.filter(file => {
try {
...
const __indexContent = successfulMigrations
      .map(file => {
const componentName = file
          ...
          .replace(/\b\w/g, l => l.toUpperCase())
          .replace(/\s/g, '')
        const __filename = file.replace(/\.html$/, '')
