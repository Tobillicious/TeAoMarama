...
const __learningPath = this.mathConcepts.find(c => c.name === concept)
    if (!learningPath) return ''

const __level = learningPath.levels[currentLevel - 1]
    if (!level) return ''
...
const __progression = this.generateSpiralProgression(concept.name, level)
...
export const __globalDepthOrchestrator = new DepthCurriculumOrchestrator()
