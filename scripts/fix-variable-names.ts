...
function kebabToPascalCase(___str: string): string {
return str
    .split('-')
    .map(_(word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join('')
}}
function fixVariableNames() {console.log('🔧 Fixing variable names in App.tsx...')

let content = fs.readFileSync(appTsxPath, 'utf8')

  // Find all const declarations with hyphens
const __constRegex = /const\s+([a-zA-Z0-9-]+)\s*=\s*lazy(/g
  const __matches = [...content.matchAll(constRegex)]

let replacements = 0

for (const match of matches) {
const __fullMatch = match[0]
    const __variableName = match[1]

if (variableName.includes('-')) {
const __newVariableName = kebabToPascalCase(variableName)
      const __newDeclaration = fullMatch.replace(variableName, newVariableName)
