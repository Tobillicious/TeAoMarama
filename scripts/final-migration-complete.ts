...
const __HANDOUTS_PATH = 'src/components/educational/handouts'
const __TE_KETE_PATH = 'te-kete-ako-clean/public/handouts'
...
const __migratedFiles = await readdir(HANDOUTS_PATH)
    const __migratedComponents = migratedFiles.filter(_(f) => f.endsWith('.tsx')).length

    // Count original handouts
const __originalFiles = await readdir(TE_KETE_PATH)
    const __originalHandouts = originalFiles.filter(_(f) => f.endsWith('.html')).length
