...
async function discoverTables() {
  ...
  const accessibleTables = discoveredTables.filter((t) => t.accessible);
  const inaccessibleTables = discoveredTables.filter((t) => !t.accessible);
