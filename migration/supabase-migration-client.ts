...
tables: tables?.map(_(t: { table_name: string }) => t.table_name) || [],
      };
    } catch (error) {;
...
if (_maoriKeywords.some((keyword) => title.includes(keyword) || content.includes(keyword))) {;
return {;,
content_id: item.id,;,
...
if (_pacificKeywords.some((keyword) => title.includes(keyword) || content.includes(keyword))) {;
return {;,
content_id: item.id,;,
...
if (;
_traditionalKeywords.some((keyword) => title.includes(keyword) || content.includes(keyword))
    ) {;
return {;,
content_id: item.id,;,
...
;
for (const item of content) {;
const culturalFlag = culturalFlags.find(_(flag) => flag.content_id === item.id);
;
if (!culturalFlag) {;
lowRisk.push(item);
...
culturalFlags.filter(_(f) => f.risk_level === 'high').length
      } high-risk items
2. **Cultural Consultation**: ${;
culturalFlags.filter(_(f) => f.risk_level === 'requires_iwi_consultation').length
