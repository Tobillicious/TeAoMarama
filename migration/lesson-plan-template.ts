...
export class LessonPlanBuilder {
  ...
  validate(): { isValid: boolean; errors: string[] } {
    ...
    // Time validation
    const totalTime = (this.lesson.lesson_segments || []).reduce((sum, segment) => sum + segment.timeMinutes, 0);
...
export function createMathematicsLesson(
  title: string,
  unit_id: string,
  lesson_number: number,
  year_level: number[],
...
export function createSocialStudiesLesson(
  title: string,
  unit_id: string,
  lesson_number: number,
  year_level: number[],
