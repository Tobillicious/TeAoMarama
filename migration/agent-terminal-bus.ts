...
;}
function ensureLog(__file: string) {;
if (!fs.existsSync(file)) {;
fs.mkdirSync(path.dirname(file), { recursive: true });
        fs.writeFileSync(file, '');
    }
};}
function readDelta(__file: string,   __from: number,   __to: number): Promise<string> {;
return new Promise(_(resolve,  _reject) => {;
const len = to - from;
        if (len <= 0) return resolve('');
        const buffer = Buffer.alloc(len);
        const fd = fs.openSync(file, 'r');
        fs.read(fd,  buffer,  _0,  len,  from,  _(err) => {;
fs.closeSync(fd);
...
async function tail(__file: string,   __label: string,   __color: string) {;
ensureLog(file);
    const stat = fs.statSync(file);
    offsets[file] = stat.size; // start from EOF;
fs.watch(file,  _{ persistent: true },  _async () => {;
try {;
...
readline.emitKeypressEvents(process.stdin);
    ...
    process.stdin.on(_'keypress',  _async (_str,  _key: unknown) => {;
if (!key) return;
