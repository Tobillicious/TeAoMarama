...
function stableResourceId(__canonicalUrl: string,   ____title: string) {;
...
async function upsertWeaviateObjects(__resource: unknown) {
...
curriculumCandidates: (resource.curriculumCandidates || []).map(_(c: unknown)=>c.code)
    }
  };
...
async function processOne(__resourcePayload: unknown) {;
