...
report.database_schema = await teKeteAkoClient.analyzeDatabaseSchema();
      const tableNames = [...new Set(_report.database_schema.map((col) => col.table_name))];
;
...
_(f) => f.risk_level === 'high' || f.risk_level === 'requires_iwi_consultation',
          );
...
console.log('\n📊 DATABASE METRICS:');
  console.log(;
_`• Total Tables: ${[...new Set(report.database_schema.map((col) => col.table_name))].length}`,
  );
  console.log(`• Total Records Found: ${report.content_inventory.total_records}`);
...
;
const highRiskFlags = report.cultural_flags.filter(_(f) => {;
...
console.log('\n🎯 IMMEDIATE ACTIONS REQUIRED:');
  report.recommendations.forEach(_(rec,  _index) => {;
