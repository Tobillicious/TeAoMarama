...
console.log('\n⚙️ Test 3: Supabase client initialization...');
  try {;
createClient(TEKETE_SUPABASE_URL, TEKETE_SUPABASE_KEY, {;,
auth: {;,
...
console.log('\n📋 DIAGNOSTIC REPORT FOR KAITIAKI MAHARA');
  console.log('='.repeat(60));
;
const successfulTests = results.filter(_(r) => r.success);
  const failedTests = results.filter(_(r) => !r.success);
;
...
console.log('\n🚨 FAILED TESTS:');
    failedTests.forEach(_(test,  _index) => {;
...
console.log('✅ ALL TESTS PASSED - Database ready for migration');
  ...
  } else if (_successfulTests.some((t) => t.test_name === 'Basic URL Connectivity')) {;
...
const failedTests = results.filter(_(r) => !r.success);
;
if (_failedTests.some((t) => t.test_name === 'Basic URL Connectivity')) {;
...
if (_failedTests.some((t) => t.test_name === 'Supabase API Endpoint')) {;
...
export { generateDiagnosticReport, runConnectionDiagnostics };
